...
DataManager.isThisGameFile = function(savefileId) {
    ...
    if (globalInfo && globalInfo[savefileId]) {
        if (StorageManager.isLocalMode()) {
			...
			//THIS CODE WAS ADDED BY NLT!!!
			
			 var savefile = globalInfo[savefileId];
            return (savefile.globalId === this._globalId &&
                    savefile.title === $dataSystem.gameTitle);
					
            //return true; <-- THIS WAS THE OLD CODE
        } else {
            var savefile = globalInfo[savefileId];
            return (savefile.globalId === this._globalId &&
                    savefile.title === $dataSystem.gameTitle);
...
DataManager.maxSavefiles = function() {
    return 40; //NLT edited from 20 to 40
};
...
ImageManager.loadNormalBitmap = function(path, hue) {
	...
	//Added by NLT!
       ...
       delete ImageManager.caching[path];
   }
	//------------
...
//THIS CODE WAS ADDED BY NLT!!!
ImageManager.cacheEmpty = function() {
    if(Object.keys(ImageManager.caching).length < 1) {
        return true;
    }
