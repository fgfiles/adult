...
function renderActionGameScene(step, action) {
    // console.log('renderActionGameScene action', action);
    if (step) {
    ...
    if (gameLineR[currentGameLine] && gameLineR[currentGameLine].actions
        ...
        if (isDev) {
            ...
            console.log('Scene', currentGameLine, gameLineR[currentGameLine].actions[currentActionGameLine].type);
        }
        ...
        function renderDialogGameScene(dialog) {
            ...
            var dialogHeight = image.indexOf('full_') ? defaultSizeDialog.height : 320;
            if (dialog.height) {
            ...
            if (dialog.image.indexOf('full_') + 1) {
                ...
                ObjAnimate(
                    currentDialog,
                    'scale_x', 0, 0, function () { },
                    [0, 0, 0, 0.5, 0, 1]
                );

                ObjAnimate(
                    currentDialog,
                    'scale_y', 0, 0, function () {
                        // console.log(dialog);
                        if (dialog.next) {
                            renderActionGameScene();
                        }
                ...
                ObjAnimate(
                    currentDialog,
                    'pos_x', 0, 0, function () { },
                    [0, 0, 'cur', timeAnimateDialog, 0, posX]
                );

                ObjAnimate(
                    currentDialog,
                    'pos_y', 0, 0, function () { },
                    [0, 0, 'cur', timeAnimateDialog, 0, posY]
                );
            } else {
                ObjAnimate(
                    currentDialog,
                    'pos_x', 0, 0, function () { },
                    [0, 0, prevDialogPosXY.x, timeAnimateDialog, 0, posX]
                );

                ObjAnimate(
                    currentDialog,
                    'pos_y', 0, 0, function () { },
                    [0, 0, prevDialogPosXY.y, timeAnimateDialog, 0, posY]
                );
            }

            ObjAnimate(currentDialog, 'alp', 0, 0, function () {
        ...
        if (gameLineR[currentGameLine].actions[currentActionGameLine].type == 'dialog') {
            renderDialogGameScene(gameLineR[currentGameLine].actions[currentActionGameLine]);
        } else if (gameLineR[currentGameLine].actions[currentActionGameLine].type == 'dialogs') {
            gameLineR[currentGameLine].actions[currentActionGameLine].dialogs.forEach((item) => {
                renderDialogGameScene(item);
            });
        }

        if (gameLineR[currentGameLine].actions[currentActionGameLine].type == 'play') {
            var posX = gameLineR[currentGameLine].actions[currentActionGameLine].x || defaultPosPlay.x;
            var posY = gameLineR[currentGameLine].actions[currentActionGameLine].y || defaultPosPlay.y;
            var image = (gameLine[currentGameLine].actions[currentActionGameLine].image);

            // ObjSet('spr_interface_play', { res: 'images/play.gif', pos_x: posX, pos_y: posY, alp: 1, input: 1,
            ObjSet('spr_interface_play', {
                pos_x: posX, pos_y: posY, alp: 1, input: 1,
                event_mup: function () {
                    ...
                    if (gameLineR[currentGameLine].actions[currentActionGameLine]) {
                        if (gameLineR[currentGameLine].actions[currentActionGameLine] && gameLineR[currentGameLine].actions[currentActionGameLine].click) {
                            gameLineR[currentGameLine].actions[currentActionGameLine].click();
                        }

                        if ((currentActionGameLine >= gameLineR[currentGameLine].actions.length - 1) && gameLineR[currentGameLine].actions[currentActionGameLine].next) {
                            return renderGameScene();
                        }
                        if (gameLineR[currentGameLine].actions[currentActionGameLine].next) {
                            renderActionGameScene();
                        }
                    } else {
                        return renderGameScene();
...
renderGameScene = function (key) {
    ...
    if (gameLine[currentGameLine] && gameLineR[currentGameLine].smoothFinish && !isCloseScene) {
        // console.log('renderGameScene animation');
        // console.log('spr_interface_video_bg');
        return ObjAnimate('spr_interface_video_bg', 'alp', 0, 0, function () {
            closeGameScene();
            renderNextScene();
        }, [0,0,0, 0.5,0,1]);
    } else {
    ...
    function closeGameScene() {
        ...
        if (gameLine[currentGameLine] && gameLine[currentGameLine]) {
            vid = document.getElementById('vid_video_' + gameLine[currentGameLine].video);
        }
        if (vid) {
            ObjSet('vid_video_' + gameLine[currentGameLine].video, { alp: 0, input: 0 });
        }
    ...
    function renderNextScene() {
        ...
        if (gameLine[tmpKey] && (gameLine[tmpKey].video.lastIndexOf('_stop') + 1) &&
            (gameLine[tmpKey].video.length - gameLine[tmpKey].video.lastIndexOf('_stop') == 5)) {
            ...
            if (key) {
                ...
                if ((gameLine[currentGameLine + 1]) && (!doneScenes['vid_video_' + gameLine[currentGameLine + 1].video])) {
                    isNeedContent = true;
                    ...
                    return pushVideosAndBackgrounds(currentGameLine + 1, currentGameLine + 7);
                }
            }
        }
        ...
        if (gameLine[currentGameLine] && gameLineR[currentGameLine].video) {
            if ((gameLine[currentGameLine].video.lastIndexOf('_stop') + 1) &&
                (gameLine[currentGameLine].video.length - gameLine[currentGameLine].video.lastIndexOf('_stop') == 5)) {
                startSexScene(
                ...
                var vid = document.getElementById('vid_video_' + gameLine[currentGameLine].video);
                ...
                vid.onended = function () {
                    // console.log('onended', gameLine[currentGameLine].video);
                    if (gameLine[currentGameLine].onended) {
                        gameLineR[currentGameLine].onended();
                        
                        if ((currentActionGameLine >= gameLineR[currentGameLine].actions.length - 1) && gameLineR[currentGameLine].next) {
                            return renderGameScene();
                        }
                ...
                ObjSet('vid_video_' + gameLine[currentGameLine].video, { input: 1, alp: 1 });
...
function createDialogs() {
    ...
    function createDialog(element) {
        var isExistsDialog = document.getElementById('spr_interface_dialog' + element.image);
        if (isExistsDialog) {
            return false;
        }
        ...
        dialogElement.style='left: 0px; top: 0px; z-index: 100; width: auto; height: auto; transform: scaleX(1) scaleY(1) rotate(0deg); pointer-events: none; opacity: 0; background-image: url(""); background-position: 0px 0px;'
        ...
        function setPercentDownloadDialogs() {
            ...
            if (isNumber(percent) && percent >= 0 && percent <= 1) {
                if (isDev) {
                    console.log('dialogs', countDownloadedDioalogs, '/', countDioalogs, percent);
                }
                ObjSet('spr_interface_loading_download_scene_text', { text: 'Loaded ' + (percent * 100).toFixed(0) + '%' });
            }
        }
