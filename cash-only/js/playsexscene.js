...
SetSmallGif = function(nameScene) {
    ...
    if (nameScene) {
        ...
        var newVideoSrcSmall = './images/' + nameVideo + '_' + currentSpeedVideo + '_small.gif';
        if (isVideoCum) {
            newVideoSrcSmall = './images/' + nameVideo + '_' + 'cum' + '_small.gif';
        } else if (isVideoAfterCum) {
            newVideoSrcSmall = './images/' + nameVideo + '_' + 'after_cum' + '_small.gif';
        }
...
function setSpeedVideo(speedVideo) {
    ...
    */
    ...
    ObjAnimate('spr_bottle_speed_' + currentSpeedVideo, 'pos_x', 0, 0, function() {
        // if (needSpeedVideo == currentSpeedVideo) {
            doneSpeeds.push(currentSpeedVideo);
            startTimerSpeed();
            // console.log('spr_bottle_speed_' + currentSpeedVideo);
...
function onClickCum() {
    ...
    if (!cum) { // Если cum отсутствует то обнуляем и рендерим следующее видео
        if (afterCum) {
            setLoading(1);
...
function createCumImages() {
    for (var key in gameLineSexScene) {
        if ( ('cum_image' in gameLineSexScene[key]) && (gameLineSexScene[key].cum_image) ) {
    ...
    for (var key in cumImages) {
        ...
        cumElement.style='left: 30px; top: 27px; z-index: 3; width: 106px; height: 106px; transform: scaleX(1) scaleY(1) rotate(0deg); pointer-events: none; opacity: 0; background-image: url("images/' + image + '");  background-position: 0px 0px; cursor: pointer;'
