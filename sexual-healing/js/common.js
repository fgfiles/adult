/* ******************** Комнаты ******************** */
// Все комнаты
var rooms = [ 'room_preloader', 'room_game', 'room_video', 'room_bottle', 'room_win', 'room_gameover' ];
...
function SwitchRoom(room) {
    ...
    ObjStopAnimate(room, "alp");
    ObjSet(room, { active: 1, alp: 1 });
    currentRoom = room;
}
...
function PlaySound(path, loop) {
    ...
    // console.warn('Попытка воспроизведения ');
    if (type && !currentLoopSounds[path]) {
        // console.warn('Воспроизведение', path);
        ...
        if (loop) {
            // console.warn('Устанавливаем флаг повторения', path);
            currentLoopSounds[path] = 1;
        }
    }
}
...
function ObjAnimate(obj, type, loop, relative, cb, anm) {
    if (anm.length % 3 == 0) {
        ...
        // Получить релевантный курсор
        function getRelevantCursor() {
            if (totalTime < timeLine[cursor]) {
                return cursor;
            }
