...
function createImg(backgrounds, room) {
  ...
  for (var element in imgs) {
    ...
    if (element && element != 'bg_black') {
      ...
      imgElement.style =
        'left: 0px; top: 0px; z-index: 0; width: 1024px; height: 576px; transform: scaleX(1) scaleY(1) rotate(0deg); pointer-events: none; opacity: 0; background-image: url(""); background-position: 0px 0px; background-color: black;';
...
function createVideo(videoArray, room) {
  ...
  for (var element in videos) {
    if (element && element != 'bg_black') {
      ...
      videoElement.style =
        'left: 0px; top: 0px; z-index: 0; width: 1024px; height: 576px; transform: scaleX(1) scaleY(1.005) rotate(0deg); pointer-events: none; opacity: 0; background-image: url(""); background-position: 0px 0px; background-color: black;';
...
function pushVideosAndBackgrounds(start, end) {
  for (var key in obj) {
    ...
    if (key >= start && key <= end) {
      if (
        scene.video.length > 5 &&
        scene.video.length - scene.video.lastIndexOf('_stop') == 5
      ) {
